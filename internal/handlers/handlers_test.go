...
func TestHandlers(t *testing.T) {
	...
	for _, e := range theTest {
		...
		if err != nil {
			t.Errorf("Error getting response from %s: %s", e.name, err)
			t.Fatal(err)
		}
...
func TestRepository_Reservations(t *testing.T) {
	...
	// Simulating what we get from request response life cycle. => response recorder == rr
	...
	if rr.Code != http.StatusOK {
		t.Errorf(" Reservation handler returned wrong status code: got %d want %d", rr.Code, http.StatusOK)
	}
	...
	if rr.Code != http.StatusTemporaryRedirect {
		t.Errorf("Reservation handler returned wrong status code: got %d want %d", rr.Code, http.StatusTemporaryRedirect)
	}
