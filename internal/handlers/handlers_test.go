...
func TestLogin(t *testing.T) {
	...
	for _, e := range loginTests {
		...
		if e.expectedLocation != "" {
			...
			if actualLocStr := actualLocation.String(); actualLocStr != e.expectedLocation {
				t.Errorf("Failed %s:expected %s, got %s", e.name, e.expectedLocation, actualLocStr)
			}
		}
		...
		if e.expectedHTML != "" {
			...
			html := rr.Body.String()
			if !strings.Contains(html, e.expectedHTML) {
				t.Errorf("Failed %s:expected %s, got %s", e.name, e.expectedHTML, html)
			}
		}
